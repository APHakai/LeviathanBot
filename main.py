...
async def api_members_list(request: Request):
    data = await request.json()
    if auth(data):
        return JSONResponse({"ok": False, "error": auth(data)}, status_code=403)

    gid = int(data.get("g") or 0)
    guild = bot.get_guild(gid) if gid else None
    if not guild:
        return JSONResponse({"ok": False, "error": "Serveur introuvable (bot offline ?)"}, status_code=404)
    ...
    if len(members) == 0:
        try:
            ...
            return JSONResponse(
                {
                    "ok": False,
                    "error": "Impossible de récupérer les membres. Vérifie SERVER MEMBERS INTENT + redeploy.",
                    "detail": str(e),
                },
                status_code=400,
            )
    ...
    return {"ok": True, "members": members[:2000]}
