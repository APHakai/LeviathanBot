...
def parse_duration_to_seconds(s: str) -> int:
    ...
    Parse durations like:
      - 10m, 2h, 1d, 1w, 1mo, 1y
      - composed: 1h30m, 2w3d, 1d6h10m
      - with spaces: "1 h 30 m"
    Units:
    ...
    pattern = re.compile(r"(\d+)(mo|[smhdwy])")
    pos = 0
    total = 0
    found = False
        ...
        <div class="card">
          ...
          <div class="row">
            <div style="flex:1; min-width:260px">
              ...
              <div class="hint" style="margin-top:6px">Astuce : tu peux aussi coller un ID directement ci-dessous (fallback).</div>
              <input id="target" placeholder="Fallback ID (optionnel)" style="margin-top:6px"/>
            </div>
            <div>
              <label>Raison</label>
          ...
          <div class="row" style="margin-top:10px">
            ...
            <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
              ...
              <input id="timeout_custom" placeholder="Ex: 1h30m / 2w3d / 10m" style="display:none; flex:1; min-width:220px;" />
            </div>
            <div class="hint">Formats: 10m, 2h, 1d, 1w, 1mo, 1y, ou composés (1h30m).</div>
          </div>
          ...
          <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;">
            ...
            <button class="btn" onclick="panelPunish('kick')">Kick</button>
...
async async function loadMembers(){
  try{
    ...
    if(d && d.ok && Array.isArray(d.members)){
      ...
      if(sel){ sel.innerHTML = '<option value="0">— Aucun membre (vérifie Members Intent) —</option>'; }
    }
  }catch(e){
...
function renderMembers(members){
  const sel = document.getElementById('target_select');
  if(!sel) return;
  sel.innerHTML = '<option value="0">— Sélectionner un membre —</option>';
...
function filterMembers(){
  const q = (document.getElementById('member_search')||{}).value || '';
  if(!window.__members_cache) return;
  const query = q.trim().toLowerCase();
  const members = window.__members_cache;
  const filtered = query
    ? members.filter(m => (m.display||'').toLowerCase().includes(query) || (m.name||'').toLowerCase().includes(query) || (m.id||'').includes(query))
    : members;
  renderMembers(filtered.slice(0, 200));
}
...
async def api_members_list():
    ...
    # Fallback: fetch members from API if cache looks empty
    if len(members) == 0:
        try:
            ...
            # Most common cause: Members Intent not enabled in Discord Developer Portal
            return JSONResponse(
                {
                    "ok": False,
                    "error": "Cannot fetch members. Enable SERVER MEMBERS INTENT in Discord Developer Portal, then redeploy.",
                    "detail": str(e),
                },
                status_code=400,
            )
