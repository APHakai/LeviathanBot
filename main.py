...
def parse_duration_to_seconds(s: str) -> Optional[int]:
    ...
    Parse duration strings like:
      10s, 5m, 2h, 1d, 2w, 1mo, 1y
      composed: 1h30m, 2w3d6h, "1 h 30 m"
    Units:
      s=seconds, m|min=minutes, h=hours, d=days, w=weeks(7d), mo=months(30d), y=years(365d)
    Returns total seconds, or None if invalid / zero.
    """
    s = (s or "").strip().lower()
    if not s:
        return None

    # Remove spaces to support "1 h 30 m"
    s = re.sub(r"\s+", "", s)
    ...
    for m in token_re.finditer(s):
        if m.start() != pos:
            return None  # invalid gap / unknown text
        n = int(m.group(1))
        unit = m.group(2)
        total += n * mult[unit]
        pos = m.end()
    ...
    <section id="tab-dashboard" class="tab active">
      <div class="grid3">
        ...
        <div class="card">
          ...
          <div class="row">
            <div>
              <label>Membre (pseudo)</label>
              <input id="member_search" placeholder="Rechercher un pseudo / @name / ID..." oninput="filterMembers()"/>
              <select id="target_select" style="margin-top:8px" onchange="document.getElementById('target').value=this.value">
                <option value="">— Sélectionner un membre —</option>
              </select>
              <div id="members_status" style="margin-top:6px;font-size:12px;opacity:.85"></div>
              <label style="margin-top:10px">ID utilisateur (fallback)</label>
          ...
          <div class="row" style="margin-top:10px">
            <div>
              ...
              <input id="timeout_custom" style="display:none;margin-top:8px" placeholder="ex: 1h30m / 2w3d / 1mo" />
            </div>
            <button class="btn" style="margin-top:22px" onclick="panelPunish('timeout')">Appliquer</button>
...
function filterMembers(){
  ...
  const list = window.__members_cache || [];
  if(!q){
    renderMembers(list);
    return;
  }
...
async function loadMembers(){
  const status = document.getElementById('members_status');
  const sel = document.getElementById('target_select');
  if(status) status.textContent = 'Chargement des membres…';
  if(sel) sel.disabled = true;

  try{
    const d = await api('/api/members/list', {k:keyVal(), g:guildVal()});
    if(d && d.ok && Array.isArray(d.members)){
      window.__members_cache = d.members;
      renderMembers(d.members);
      if(status) status.textContent = d.members.length + ' membre(s) chargé(s).';
    } else {
      console.warn('Members API error', d);
      window.__members_cache = [];
      if(sel) sel.innerHTML = '<option value="">— Impossible de charger —</option>';
      if(status) status.textContent = (d && d.error) ? ('Erreur: ' + d.error) : 'Erreur inconnue.';
    }
  } catch(e){
    console.warn('loadMembers failed', e);
    window.__members_cache = [];
    if(sel) sel.innerHTML = '<option value="">— Erreur API —</option>';
    if(status) status.textContent = 'Erreur API (voir console/logs).';
  } finally {
...
async function panelPunish(action){
  const selVal = (document.getElementById('target_select')?.value || '').trim();
  const target = (selVal || document.getElementById('target').value || '').trim();
...
async def api_members_list(request: Request):
    data = await request.json()
    if not valid_admin_key(data.get("k")):
        return JSONResponse({"ok": False, "error": "Bad key"}, status_code=403)

    gid = data.get("g")
    if not gid:
        return JSONResponse({"ok": False, "error": "Missing guild id"}, status_code=400)

    guild = bot.get_guild(int(gid))
    if not guild:
        return JSONResponse({"ok": False, "error": "Guild not found (bot not in guild / not ready)"}, status_code=404)
    ...
    # Fallback: fetch via API (still requires intents for consistent large guild results)
    if not members:
        try:
            ...
            return JSONResponse({"ok": False, "error": "Cannot fetch members", "detail": str(e)}, status_code=400)
    ...
    try:
        ...
        if action in ("kick", "ban", "timeout"):
            ...
            if action == "timeout":
                ...
                add_infraction(gid, uid, None, "timeout", reason)
