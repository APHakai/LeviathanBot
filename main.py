...
# Explicitly ensure privileged intents are enabled in code
...
def parse_duration_to_seconds(s: str) -> Optional[int]:
    ...
    Parse durations like:
      10m, 2h, 1d, 1w, 1mo, 1y
      1h30m, 2w3d, 1 d 6 h, etc.
    Units:
      s=seconds, m|min=minutes, h=hours, d=days, w=weeks(7d), mo=months(30d), y=years(365d)
    ...
    if not found or pos != len(compact):
        return None
    return total
        ...
        <div class="card">
          ...
          <div class="row">
            ...
            <div style="min-width:260px;flex:1">
              ...
              <select id="target_select" onchange="syncTargetFromSelect()">
                <option value="0">— Charge d'abord (bouton Charger) —</option>
              </select>
              <div class="hint" id="members_status" style="margin-top:6px;opacity:.85"></div>
            </div>
          </div>

          <div class="row" style="margin-top:10px">
            <div style="min-width:260px;flex:1">
              <label>ID utilisateur (fallback)</label>
              <input id="target" placeholder="ex: 1234567890 (si besoin)"/>
            </div>
            <div style="min-width:260px;flex:1">
              <label>Raison</label>
          ...
          <div class="row" style="margin-top:10px">
            <div style="min-width:260px;flex:1">
              ...
              <div class="row" style="gap:8px">
                <select id="timeout_preset" onchange="toggleTimeoutCustom()">
                ...
                <input id="timeout_custom" placeholder="ex: 1h30m / 2w3d" style="display:none;max-width:220px" />
              </div>
              <div class="hint" style="margin-top:6px;opacity:.85">Max Discord: ~28 jours. Formats: 10m, 2h, 1d, 1w, 1mo, 1y, 1h30m…</div>
            </div>

            <div style="min-width:260px;flex:1;display:flex;align-items:flex-end;gap:8px">
...
// ===== Members dropdown + timeout duration UI =====
let _membersCache = [];

function toggleTimeoutCustom(){
  const p = document.getElementById('timeout_preset');
  const c = document.getElementById('timeout_custom');
  if(!p || !c) return;
  c.style.display = (p.value === 'custom') ? 'block' : 'none';
}

function getSelectedTargetId(){
  const sel = document.getElementById('target_select');
  const fallback = document.getElementById('target');
  const vSel = sel ? (sel.value||'').trim() : '';
  if(vSel && vSel !== '0') return vSel;
  const vFb = fallback ? (fallback.value||'').trim() : '';
  return vFb;
}
...
function syncTargetFromSelect(){
  const sel = document.getElementById('target_select');
  const fallback = document.getElementById('target');
  if(sel && fallback && sel.value && sel.value !== '0'){
    fallback.value = sel.value;
  }
}

function renderMembers(list){
  const sel = document.getElementById('target_select');
  const st = document.getElementById('members_status');
  if(!sel) return;

  if(!Array.isArray(list) || list.length === 0){
    sel.innerHTML = '<option value="0">— Aucun membre —</option>';
    if(st) st.innerText = 'Aucun membre reçu (vérifie Members Intent + bot dans ce serveur).';
    return;
  }

  const maxShow = 200;
  const shown = list.slice(0, maxShow);
  sel.innerHTML = '<option value="0">— Sélectionner un membre —</option>' + shown.map(m => (
    `<option value="${m.id}">${escapeAll(m.display || m.name || m.id)}</option>`
  )).join('');
  if(st) st.innerText = `${list.length} membre(s) chargé(s) (affichage ${shown.length}).`;
}

function filterMembers(){
  const q = (document.getElementById('member_search')?.value || '').trim().toLowerCase();
  if(!q){
    renderMembers(_membersCache);
    return;
  }
  const filtered = _membersCache.filter(m => {
    ...
    return d.includes(q) || n.includes(q) || id.includes(q);
  });
...
async function loadMembers(){
  const st = document.getElementById('members_status');
  if(st) st.innerText = 'Chargement des membres…';
  const d = await api('/api/members/list', {k:keyVal(), g:guildVal()});
  if(d && d.ok && Array.isArray(d.members)){
    _membersCache = d.members;
    renderMembers(_membersCache);
    if(st) st.innerText = `✅ ${d.count||_membersCache.length} membre(s) chargé(s) (${d.used||'?'})`;
  } else {
    console.warn('Members API error', d);
    _membersCache = [];
    renderMembers([]);
    if(st) st.innerText = (d && (d.error || d.detail)) ? ('Erreur: ' + (d.error || d.detail)) : 'Erreur de chargement des membres.';
  }
}
...
async function panelPunish(action){
  const target = getSelectedTargetId();
  const reason = document.getElementById('reason').value.trim() || 'Via Panel';

  let payload = {k:keyVal(), g:guildVal(), action:action, target:target, reason:reason};
  ...
  if(action === 'timeout'){
    ...
    const duration = (preset === 'custom') ? custom : preset;
...
try{
  document.getElementById('guild').addEventListener('change', async ()=>{ await loadMembers(); });
}catch(e){}
</script>
</body>
</html>
...
async def api_debug(request: Request):
    ...
    return {
        ...
        "latency_ms": int(getattr(bot, "latency", 0.0) * 1000),
        "intents": {
            "members": bool(getattr(getattr(bot, "intents", None), "members", False)),
            "guilds": bool(getattr(getattr(bot, "intents", None), "guilds", False)),
            "message_content": bool(getattr(getattr(bot, "intents", None), "message_content", False)),
        },
        "guilds": [{"id": str(g.id), "name": g.name, "member_count": getattr(g, "member_count", None)} for g in guilds],
    }
}
...
async def api_members_list(request: Request):
    ...
    if not guild:
        return JSONResponse({"ok": False, "error": "Serveur introuvable (bot offline / pas prêt ?)"}, status_code=404)

    used = "cache"
    # Ensure cache is populated when possible
    try:
    ...
    members = []
    # Prefer cached members first
    try:
        for mbr in guild.members:
            if getattr(mbr, "bot", False):
                continue
            display = f"{mbr.display_name} (@{mbr.name})"
            members.append({"id": str(mbr.id), "name": mbr.name, "display": display})
    except Exception:
        members = []

    # Fallback: fetch from API (requires Members Intent)
    if len(members) == 0:
        used = "fetch"
        try:
            ...
            return JSONResponse(
                {
                    "ok": False,
                    "error": "Impossible de récupérer les membres via l’API Discord.",
                    "hint": "Vérifie: (1) SERVER MEMBERS INTENT activé + redeploy, (2) bon TOKEN, (3) bot présent sur le serveur, (4) bot connecté.",
                    "detail": str(e),
                    "debug": {
                        "bot_connected": bool(bot.user),
                        "intents_members": bool(getattr(getattr(bot, "intents", None), "members", False)),
    ...
    return {
        ...
        "debug": {
            ...
            "guild_id": str(guild.id),
            "guild_name": guild.name,
            "cached_total": len(getattr(guild, "members", []) or []),
        },
    }
    ...
    try:
        ...
        if action in ("kick", "ban", "timeout"):
            ...
            if action == "timeout":
                ...
                secs = parse_duration_to_seconds(duration_raw)
                if not secs:
                    return {"error": "Durée invalide. Ex: 10m, 2h, 1d, 1w, 1mo, 1y, 1h30m"}
                if secs > 28 * 86400:
                    return {"error": "Durée trop longue (max ~28 jours pour Discord)."}
                ...
                await send_modlog(guild, f"🤐 Timeout {duration_raw}: {member.mention} | {reason}")
                return {"details": "Timeout OK."}
...
async def on_ready():
    ...
    # Preload member cache for panel dropdown
    try:
        for _g in list(getattr(bot, "guilds", []) or []):
            try:
                await _g.chunk(cache=True)
            except Exception:
                pass
    except Exception:
        pass
