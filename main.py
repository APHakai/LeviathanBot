...
# =========================================================
# LEVIATHAN ULTRA PANEL PATCH
# =========================================================
def ultra_panel_patch():
    global PANEL_HTML
    if 'Leviathan Ultra Suite' in PANEL_HTML:
        return
    PANEL_HTML = PANEL_HTML.replace('</style>', """
    .ultra-grid4{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin:16px 0}
    .ultra-kpi .value{font-size:30px;font-weight:800;margin-top:6px}
    .ultra-kpi .sub{font-size:12px;color:var(--muted);margin-top:6px}
    .ultra-split{display:grid;grid-template-columns:1.2fr 1fr;gap:14px}
    .ultra-table{display:grid;gap:10px}
    .ultra-row{display:grid;grid-template-columns:1fr auto auto;gap:10px;padding:12px;border:1px solid rgba(255,255,255,.06);border-radius:14px;background:rgba(255,255,255,.03)}
    .ultra-badge{display:inline-flex;align-items:center;justify-content:center;padding:5px 10px;border-radius:999px;font-size:11px;font-weight:700}
    .ultra-badge.ok{background:rgba(53,255,155,.10);color:#b4ffd7;border:1px solid rgba(53,255,155,.18)}
    .ultra-badge.warn{background:rgba(255,209,102,.10);color:#ffe2a3;border:1px solid rgba(255,209,102,.18)}
    .ultra-badge.bad{background:rgba(255,59,92,.10);color:#ffcad5;border:1px solid rgba(255,59,92,.18)}
    .ultra-mini{display:flex;gap:8px;flex-wrap:wrap}
    .ultra-mini .btn{flex:none;min-width:unset}
    @media(max-width:1180px){.ultra-split,.ultra-grid4{grid-template-columns:1fr 1fr}}
    @media(max-width:760px){.ultra-split,.ultra-grid4{grid-template-columns:1fr}}
    </style>""")
    ...
    # add more tabs if not already present
    if 'tab-serverplus' not in PANEL_HTML:
        PANEL_HTML = PANEL_HTML.replace(
            '<button data-tab="tab-tools">Outils</button>',
            '<button data-tab="tab-serverplus">Server Manager</button>\n      <button data-tab="tab-automationplus">Automation</button>\n      <button data-tab="tab-customplus">Custom+</button>\n      <button data-tab="tab-tools">Outils</button>'
        )
    dash_start = PANEL_HTML.find('<section id="tab-dashboard" class="tab active">')
    dash_end = PANEL_HTML.find('</section>', dash_start)
    if dash_start != -1 and dash_end != -1:
        PANEL_HTML = PANEL_HTML[:dash_start] + """
    <section id="tab-dashboard" class="tab active">
      <div class="card">
        <div class="title">Leviathan Ultra Suite</div>
        <div style="font-size:28px;font-weight:800;margin-bottom:10px">Dashboard unifié inspiré des gros bots publics</div>
        <div class="hint">Vue d'ensemble, gestion communauté, automatisations, custom commands, tickets, rôles, économie, logs et analytics dans un seul panel.</div>
        <div class="ultra-mini" style="margin-top:14px">
          <button class="btn primary" onclick="loadAll()">Tout recharger</button>
          <button class="btn" onclick="loadOverview()">Overview</button>
          <button class="btn" onclick="fetchInfo()">Santé bot</button>
        </div>
      </div>

      <div class="ultra-grid4">
        <div class="card ultra-kpi"><div class="title">Membres</div><div class="value" id="kpiMembers">-</div><div class="sub">Population du serveur</div></div>
        <div class="card ultra-kpi"><div class="title">Humains / Bots</div><div class="value" id="kpiHumansBots">-</div><div class="sub">Répartition</div></div>
        <div class="card ultra-kpi"><div class="title">Salons / Rôles</div><div class="value" id="kpiRoomsRoles">-</div><div class="sub">Structure</div></div>
        <div class="card ultra-kpi"><div class="title">Shop / XP top</div><div class="value" id="kpiShopXp">-</div><div class="sub">Activité</div></div>
      </div>

      <div class="ultra-split">
        <div class="card">
          <div class="title">Actions rapides</div>
          <div class="ultra-mini">
            <button class="btn danger" onclick="panelAction('lockdown', true)">Lockdown</button>
            <button class="btn" onclick="panelAction('lockdown', false)">Unlock</button>
            <button class="btn danger" onclick="panelAction('purge_global', 20)">Purge 20</button>
            <button class="btn" onclick="loadLogsOnce()">Logs</button>
          </div>
          <div class="hint" style="margin-top:10px">Pense à vérifier l'état du bot avant les actions lourdes.</div>
        </div>
        <div class="card">
          <div class="title">Résumé live</div>
          <div class="ultra-table">
            <div class="ultra-row"><div><b>Connexion bot</b><div class="hint">État Discord</div></div><div id="dashBotState">unknown</div><div><span class="ultra-badge warn" id="dashBotBadge">En attente</span></div></div>
            <div class="ultra-row"><div><b>Guilds visibles</b><div class="hint">Serveurs disponibles</div></div><div id="dashGuilds">0</div><div><span class="ultra-badge ok">Panel OK</span></div></div>
            <div class="ultra-row"><div><b>Uptime</b><div class="hint">Temps en ligne</div></div><div id="dashUptime">-</div><div><span class="ultra-badge ok">Live</span></div></div>
          </div>
        </div>
      </div>
      ...
      <div class="grid" style="margin-top:14px">
        <div class="card">
          ...
          <div class="ultra-mini" style="margin-top:10px">
            <button class="btn" onclick="panelPunishEx('timeout')">Timeout</button>
            <button class="btn" onclick="panelPunishEx('untimeout')">Un-timeout</button>
            <button class="btn" onclick="panelPunishEx('warn')">Warn</button>
            <button class="btn danger" onclick="panelPunishEx('kick')">Kick</button>
        ...
        <div class="card">
          <div class="title">Journal rapide</div>
          <div class="console" id="dashboardLogBox">Charge le serveur pour voir un résumé live.</div>
        </div>
      </div>
    </section>
""" + PANEL_HTML[dash_end+10:]

    extra_tabs = """
    <section id=\"tab-serverplus\" class=\"tab\">
      <div class=\"grid\">
        <div class=\"card\"><div class=\"title\">Server Manager</div><div class=\"hint\">Overview serveur, sticky messages, rewards de niveaux et outils de structure.</div><div class=\"ultra-mini\" style=\"margin-top:12px\"><button class=\"btn\" onclick=\"loadOverview()\">Overview</button></div><div class=\"console\" id=\"serverPlusBox\">—</div></div>
        <div class=\"card\"><div class=\"title\">État général</div><div class=\"hint\">Section prête à accueillir encore plus de gestion serveur.</div></div>
      </div>
    </section>
    <section id=\"tab-automationplus\" class=\"tab\">
      <div class=\"grid\">
        <div class=\"card\"><div class=\"title\">Automation</div><label>Trigger auto-response</label><input id=\"auto_trigger\" placeholder=\"mot-clé\"/><label>Réponse</label><textarea id=\"auto_response\" placeholder=\"réponse automatique\"></textarea><label><input type=\"checkbox\" id=\"auto_exact\"/> Match exact</label><div class=\"ultra-mini\" style=\"margin-top:12px\"><button class=\"btn primary\" onclick=\"saveAutoResponse()\">Ajouter auto-response</button><button class=\"btn\" onclick=\"loadAutoResponses()\">Lister</button></div></div>
        <div class=\"card\"><div class=\"title\">Résultats</div><div class=\"console\" id=\"automationBox\">—</div></div>
      </div>
    </section>
    <section id=\"tab-customplus\" class=\"tab\">
      <div class=\"grid\">
        <div class=\"card\"><div class=\"title\">Custom Commands</div><label>Trigger</label><input id=\"cc_trigger\" placeholder=\"ex: regles\"/><label>Réponse</label><textarea id=\"cc_response\" placeholder=\"texte de réponse\"></textarea><div class=\"ultra-mini\" style=\"margin-top:12px\"><button class=\"btn primary\" onclick=\"saveCustomCommand()\">Ajouter</button><button class=\"btn\" onclick=\"listCustomCommands()\">Lister</button></div></div>
        <div class=\"card\"><div class=\"title\">Résultats</div><div class=\"console\" id=\"customBox\">—</div></div>
      </div>
    </section>
"""
    if 'tab-serverplus' not in PANEL_HTML:
        PANEL_HTML = PANEL_HTML.replace('<section id="tab-tools" class="tab">', extra_tabs + '\n    <section id="tab-tools" class="tab">')

    PANEL_HTML = PANEL_HTML.replace('</script>', """
async function loadOverview(){
  try{
    const d = await api('/api/stats/overview',{k:keyVal(),g:guildVal()});
    if(d.error) return alert(d.error);
    const set = (id,val)=>{ const el=document.getElementById(id); if(el) el.innerText=val; };
    set('kpiMembers', d.members ?? '-');
    set('kpiHumansBots', `${d.humans ?? '-'} / ${d.bots ?? '-'}`);
    set('kpiRoomsRoles', `${((d.text_channels ?? 0)+(d.voice_channels ?? 0))} / ${d.roles ?? '-'}`);
    set('kpiShopXp', `${d.shop_items ?? 0} / ${(d.xp_top||[]).length}`);
    const box = document.getElementById('serverPlusBox');
    if(box){ box.innerHTML = [`Serveur: ${d.name}`,`Membres: ${d.members}`,`Humains: ${d.humans} | Bots: ${d.bots}`,`Salons texte: ${d.text_channels} | vocaux: ${d.voice_channels}`,`Rôles: ${d.roles}`].map(escapeHtml).join('<br/>'); }
  }catch(e){}
}
async function saveCustomCommand(){ const d=await api('/api/customcommands/add',{k:keyVal(),g:guildVal(),trigger:document.getElementById('cc_trigger').value,response:document.getElementById('cc_response').value}); if(d.error) return alert(d.error); alert('Commande ajoutée'); }
async function listCustomCommands(){ const d=await api('/api/customcommands/list',{k:keyVal(),g:guildVal()}); if(d.error) return alert(d.error); document.getElementById('customBox').innerHTML=(d.items||[]).map(x=>`!${escapeHtml(x.trigger)} → ${escapeHtml(x.response)}`).join('<br/>') || 'Aucune.'; }
async function saveAutoResponse(){ const d=await api('/api/autoresponses/add',{k:keyVal(),g:guildVal(),trigger:document.getElementById('auto_trigger').value,response:document.getElementById('auto_response').value,exact_match:document.getElementById('auto_exact').checked}); if(d.error) return alert(d.error); alert('Auto-response ajoutée'); }
async function loadAutoResponses(){ const d=await api('/api/autoresponses/list',{k:keyVal(),g:guildVal()}); if(d.error) return alert(d.error); document.getElementById('automationBox').innerHTML=(d.items||[]).map(x=>`${escapeHtml(x.trigger)} → ${escapeHtml(x.response)}`).join('<br/>') || 'Aucune.'; }
(function(){ const _oldLoadAll = typeof loadAll === 'function' ? loadAll : null; if(_oldLoadAll){ loadAll = async function(){ await _oldLoadAll(); try{ const botState=(document.getElementById('botState')||{}).innerText||'unknown'; const guilds=(document.getElementById('guildCount')||{}).innerText||'0'; const uptime=(document.getElementById('uptime')||{}).innerText||'-'; const connected=botState==='connected'; const setText=(id,val)=>{ const el=document.getElementById(id); if(el) el.innerText=val; }; setText('dashBotState',botState); setText('dashGuilds',guilds); setText('dashUptime',uptime); const badge=document.getElementById('dashBotBadge'); if(badge){ badge.className = connected ? 'ultra-badge ok' : 'ultra-badge bad'; badge.innerText = connected ? 'Connecté' : 'Offline'; } await loadOverview(); const r=await fetch('/api/logs'); const arr=await r.json(); const box=document.getElementById('dashboardLogBox'); if(box && Array.isArray(arr)){ box.innerHTML = arr.slice().reverse().slice(0,12).map(x=>`<div>${escapeHtml(x)}</div>`).join(''); } }catch(e){} } }})();
</script>""")

# lightweight extra db

def ultra_db_init():
    con = db_connect(); cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS custom_commands (guild_id INTEGER NOT NULL, trigger TEXT NOT NULL, response TEXT NOT NULL, PRIMARY KEY (guild_id, trigger))")
    cur.execute("CREATE TABLE IF NOT EXISTS auto_responses (guild_id INTEGER NOT NULL, trigger TEXT NOT NULL, response TEXT NOT NULL, exact_match INTEGER DEFAULT 0, PRIMARY KEY (guild_id, trigger))")
    con.commit(); con.close()

def cc_add(gid, trigger, response):
    con = db_connect(); cur = con.cursor(); cur.execute("INSERT OR REPLACE INTO custom_commands(guild_id,trigger,response) VALUES (?,?,?)", (gid, trigger.lower().strip(), response)); con.commit(); con.close()

def cc_list(gid):
    return _db_all("SELECT * FROM custom_commands WHERE guild_id=? ORDER BY trigger ASC", (gid,)) if '_db_all' in globals() else []

def ar_add(gid, trigger, response, exact):
    con = db_connect(); cur = con.cursor(); cur.execute("INSERT OR REPLACE INTO auto_responses(guild_id,trigger,response,exact_match) VALUES (?,?,?,?)", (gid, trigger.lower().strip(), response, exact)); con.commit(); con.close()

def ar_list(gid):
    return _db_all("SELECT * FROM auto_responses WHERE guild_id=? ORDER BY trigger ASC", (gid,)) if '_db_all' in globals() else []

def _db_all(query, params=()):
    con = db_connect(); cur = con.cursor(); cur.execute(query, params); rows = cur.fetchall(); con.close(); return [dict(r) for r in rows]

@app.post('/api/customcommands/add')
async def api_customcommands_add(request: Request):
    data = await request.json();
    if auth(data): return JSONResponse({'error': auth(data)}, status_code=403)
    gid = int(data.get('g') or 0); trig = str(data.get('trigger') or '').strip(); resp = str(data.get('response') or '').strip()
    if gid <= 0 or not trig or not resp: return {'error':'Paramètres invalides.'}
    cc_add(gid, trig, resp); return {'ok': True}

@app.post('/api/customcommands/list')
async def api_customcommands_list(request: Request):
    data = await request.json();
    if auth(data): return JSONResponse({'error': auth(data)}, status_code=403)
    gid = int(data.get('g') or 0)
    if gid <= 0: return {'error':'Guild invalide.'}
    return {'items': cc_list(gid)}

@app.post('/api/autoresponses/add')
async def api_autoresponses_add(request: Request):
    data = await request.json();
    if auth(data): return JSONResponse({'error': auth(data)}, status_code=403)
    gid = int(data.get('g') or 0); trig = str(data.get('trigger') or '').strip(); resp = str(data.get('response') or '').strip(); exact = 1 if data.get('exact_match') else 0
    if gid <= 0 or not trig or not resp: return {'error':'Paramètres invalides.'}
    ar_add(gid, trig, resp, exact); return {'ok': True}

@app.post('/api/autoresponses/list')
async def api_autoresponses_list(request: Request):
    data = await request.json();
    if auth(data): return JSONResponse({'error': auth(data)}, status_code=403)
    gid = int(data.get('g') or 0)
    if gid <= 0: return {'error':'Guild invalide.'}
    return {'items': ar_list(gid)}

try:
    ultra_db_init()
    ultra_panel_patch()
except Exception as _e:
    print('ultra patch err:', _e, flush=True)


# =========================================================
# D Y N O   S T Y L E   P A N E L   U P G R A D E
# =========================================================
def leviathan_dyno_style_patch():
    global PANEL_HTML
    if "leviathan-dyno-shell" in PANEL_HTML:
        return

    PANEL_HTML = PANEL_HTML.replace(
        "</style>",
        """
    .leviathan-dyno-shell{
      --surface:#0b1220; --surface2:#10192c; --surface3:#0f1728;
      --line:rgba(255,255,255,.08); --line2:rgba(255,255,255,.05);
      --soft:rgba(255,255,255,.04); --soft2:rgba(255,255,255,.03);
      --blue2:#6cb7ff; --green2:#53ffb1; --danger2:#ff5d78; --warning2:#ffd479;
    }
    .leviathan-dyno-shell .wrap{
      grid-template-columns: 300px 1fr;
      background:
        radial-gradient(1200px 800px at 100% -10%, rgba(83,255,177,.12), transparent 40%),
        radial-gradient(1000px 700px at -10% 10%, rgba(108,183,255,.13), transparent 35%),
        var(--bg);
    }
    .leviathan-dyno-shell .side{background:linear-gradient(180deg, rgba(10,15,25,.98), rgba(7,10,16,.98));border-right:1px solid var(--line);padding:24px 18px}
    .leviathan-dyno-shell .logo{width:44px;height:44px;border-radius:16px;box-shadow:0 12px 28px rgba(0,0,0,.35), inset 0 1px 0 rgba(255,255,255,.08)}
    .leviathan-dyno-shell .brand h1{font-size:18px}
    .leviathan-dyno-shell .pill{background:rgba(255,255,255,.04);border-color:var(--line)}
    .leviathan-dyno-shell .nav{gap:8px}
    .leviathan-dyno-shell .nav button{display:flex;align-items:center;justify-content:space-between;padding:13px 14px;border-radius:16px;background:var(--soft2);border:1px solid var(--line2);font-weight:700}
    .leviathan-dyno-shell .nav button.active{background:linear-gradient(135deg, rgba(108,183,255,.16), rgba(83,255,177,.12));border-color:rgba(108,183,255,.22);box-shadow:0 10px 24px rgba(0,0,0,.22)}
    .leviathan-dyno-shell .nav-group{font-size:11px;text-transform:uppercase;letter-spacing:.12em;color:var(--muted);opacity:.8;margin:14px 4px 6px}
    .leviathan-dyno-shell .nav .sub{color:var(--muted);font-size:11px;font-weight:600}
    .leviathan-dyno-shell .main{padding:24px}
    .leviathan-dyno-shell .card{background:linear-gradient(180deg, rgba(11,18,32,.95), rgba(14,23,40,.92));border:1px solid var(--line);border-radius:22px;padding:18px;box-shadow:0 16px 36px rgba(0,0,0,.28)}
    .leviathan-dyno-shell .topbar{display:grid;grid-template-columns:1.5fr .8fr;gap:16px;align-items:stretch;margin-bottom:16px}
    .leviathan-dyno-shell .topbar .left{display:block}
    .leviathan-dyno-shell .topbar .left .card{display:grid !important;grid-template-columns:240px 1fr auto;gap:12px;align-items:end !important}
    .leviathan-dyno-shell .hero{display:grid;grid-template-columns:1.25fr .95fr;gap:16px;margin-bottom:16px}
    .leviathan-dyno-shell .hero-primary{position:relative;overflow:hidden;min-height:190px}
    .leviathan-dyno-shell .hero-primary:before{content:'';position:absolute;inset:-20% auto auto -10%;width:280px;height:280px;background:radial-gradient(circle, rgba(108,183,255,.17), transparent 62%)}
    .leviathan-dyno-shell .hero-primary:after{content:'';position:absolute;right:-60px;bottom:-80px;width:250px;height:250px;background:radial-gradient(circle, rgba(83,255,177,.12), transparent 62%)}
    .leviathan-dyno-shell .eyebrow{display:inline-flex;align-items:center;gap:8px;padding:7px 11px;border-radius:999px;background:rgba(255,255,255,.04);border:1px solid var(--line);font-size:12px;color:var(--muted);margin-bottom:14px}
    .leviathan-dyno-shell .hero h2{font-size:30px;line-height:1.03;margin:0 0 10px 0;letter-spacing:-.03em;max-width:12ch}
    .leviathan-dyno-shell .hero p{margin:0;color:#d4e2ff;max-width:68ch;line-height:1.5}
    .leviathan-dyno-shell .action-row,.leviathan-dyno-shell .mini-actions{display:flex;gap:8px;flex-wrap:wrap}
    .leviathan-dyno-shell .action-row .btn,.leviathan-dyno-shell .mini-actions .btn{flex:none;min-width:unset}
    .leviathan-dyno-shell .stats-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-bottom:16px}
    .leviathan-dyno-shell .kpi{min-height:118px;position:relative;overflow:hidden}
    .leviathan-dyno-shell .kpi:after{content:'';position:absolute;width:130px;height:130px;right:-26px;bottom:-42px;background:radial-gradient(circle, rgba(108,183,255,.10), transparent 63%)}
    .leviathan-dyno-shell .kpi .value{font-size:30px;font-weight:800;margin-top:8px;letter-spacing:-.03em}
    .leviathan-dyno-shell .kpi .meta{color:var(--muted);font-size:12px;margin-top:8px}
    .leviathan-dyno-shell .split-2{display:grid;grid-template-columns:1.1fr .9fr;gap:16px;margin-bottom:16px}
    .leviathan-dyno-shell .ops-table{display:grid;gap:10px}
    .leviathan-dyno-shell .ops-row{display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:12px 14px;background:rgba(255,255,255,.03);border:1px solid var(--line2);border-radius:16px}
    .leviathan-dyno-shell .badge{display:inline-flex;align-items:center;justify-content:center;padding:6px 10px;border-radius:999px;font-size:11px;font-weight:800;border:1px solid transparent}
    .leviathan-dyno-shell .badge.ok{background:rgba(83,255,177,.10);color:#b8ffd9;border-color:rgba(83,255,177,.18)}
    .leviathan-dyno-shell .badge.warn{background:rgba(255,212,121,.10);color:#ffe7b1;border-color:rgba(255,212,121,.18)}
    .leviathan-dyno-shell .badge.bad{background:rgba(255,93,120,.10);color:#ffd0d8;border-color:rgba(255,93,120,.18)}
    .leviathan-dyno-shell .section-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}
    .leviathan-dyno-shell .section-head h3{margin:0;font-size:15px}
    .leviathan-dyno-shell .muted-sm{color:var(--muted);font-size:12px}
    .leviathan-dyno-shell .console{min-height:250px;height:auto;max-height:340px;border-radius:18px}
    .leviathan-dyno-shell .title{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}
    .leviathan-dyno-shell label{font-size:12px;font-weight:700;color:#b2c3e4}
    .leviathan-dyno-shell input,.leviathan-dyno-shell select,.leviathan-dyno-shell textarea{border-radius:16px;border:1px solid var(--line);background:rgba(255,255,255,.04);padding:12px 13px}
    .leviathan-dyno-shell .btn{border-radius:16px;font-weight:800}
    .leviathan-dyno-shell .side-footer{margin-top:16px;border-top:1px solid var(--line2);padding-top:16px;color:var(--muted);font-size:12px;line-height:1.45}
    .leviathan-dyno-shell .side-footer code{color:#dce8ff}
    @media(max-width:1200px){.leviathan-dyno-shell .topbar,.leviathan-dyno-shell .hero,.leviathan-dyno-shell .split-2{grid-template-columns:1fr}.leviathan-dyno-shell .stats-grid{grid-template-columns:1fr 1fr}.leviathan-dyno-shell .topbar .left .card{grid-template-columns:1fr}}
    @media(max-width:760px){.leviathan-dyno-shell .stats-grid{grid-template-columns:1fr}.leviathan-dyno-shell .ops-row{grid-template-columns:1fr}}
  </style>"""
    )
    PANEL_HTML = PANEL_HTML.replace("<body>", '<body class="leviathan-dyno-shell">')
    PANEL_HTML = re.sub(
        r'<div class="nav">.*?</div>\s*\n\n    <div style="margin-top:14px" class="hint">',
        """
    <div class="nav">
      <div class="nav-group">Vue d’ensemble</div>
      <button class="active" data-tab="tab-dashboard"><span>Dashboard</span><span class="sub">overview</span></button>
      <button data-tab="tab-analytics"><span>Analytics</span><span class="sub">health</span></button>
      <div class="nav-group">Communauté</div>
      <button data-tab="tab-config"><span>Config</span><span class="sub">guild</span></button>
      <button data-tab="tab-addonsplus"><span>Addons+</span><span class="sub">automod</span></button>
      <button data-tab="tab-moderation"><span>Modération</span><span class="sub">actions</span></button>
      <button data-tab="tab-ticketsplus"><span>Tickets+</span><span class="sub">support</span></button>
      <button data-tab="tab-reactionroles"><span>Reaction Roles</span><span class="sub">roles</span></button>
      <div class="nav-group">Contenu</div>
      <button data-tab="tab-embed"><span>Embeds</span><span class="sub">annonces</span></button>
      <button data-tab="tab-giveaway"><span>Giveaways</span><span class="sub">events</span></button>
      <button data-tab="tab-economyplus"><span>Économie+</span><span class="sub">currency</span></button>
      <div class="nav-group">Système</div>
      <button data-tab="tab-tools"><span>Outils</span><span class="sub">utils</span></button>
      <button data-tab="tab-logs"><span>Logs</span><span class="sub">live</span></button>
    </div>
    ...
    PANEL_HTML = PANEL_HTML.replace("<b>Connexion</b><br/>", "<b>Accès admin</b><br/>")
    PANEL_HTML = PANEL_HTML.replace("</aside>", "</div></aside>", 1)
    ...
    <section id="tab-dashboard" class="tab active">
      <div class="hero">
        <div class="card hero-primary">
          <div class="eyebrow">● Leviathan Control Center</div>
          <h2>Un panel plus premium, plus structuré et plus proche des gros bots publics.</h2>
          <p>Vue globale, modération rapide, sécurité, économie, tickets, rôles, analytics et outils d’administration dans une interface unique, plus claire et plus cohérente.</p>
          <div class="action-row" style="margin-top:18px">
            <button class="btn primary" onclick="loadAll()">Tout recharger</button>
            <button class="btn" onclick="loadOverview()">Rafraîchir overview</button>
            <button class="btn" onclick="loadHealthApi()">Health API</button>
          </div>
        </div>
        <div class="card">
          <div class="section-head"><h3>Résumé opérationnel</h3><span class="badge warn" id="dynoBotBadge">En attente</span></div>
          <div class="ops-table">
            <div class="ops-row"><div><b>Connexion bot</b><div class="muted-sm">Présence Discord</div></div><div id="dynoBotState">unknown</div><div><span class="badge ok">Panel actif</span></div></div>
            <div class="ops-row"><div><b>Guilds visibles</b><div class="muted-sm">Serveurs accessibles</div></div><div id="dynoGuilds">0</div><div><span class="badge ok">Live</span></div></div>
            <div class="ops-row"><div><b>Uptime</b><div class="muted-sm">Temps en ligne</div></div><div id="dynoUptime">-</div><div><span class="badge ok">OK</span></div></div>
          </div>
        </div>
      </div>
      <div class="stats-grid">
        <div class="card kpi"><div class="title">Membres</div><div class="value" id="dynoMembers">-</div><div class="meta">Population totale du serveur</div></div>
        <div class="card kpi"><div class="title">Humains / Bots</div><div class="value" id="dynoHumansBots">-</div><div class="meta">Répartition de la communauté</div></div>
        <div class="card kpi"><div class="title">Salons / Rôles</div><div class="value" id="dynoRoomsRoles">-</div><div class="meta">Structure du serveur</div></div>
        <div class="card kpi"><div class="title">Shop / XP top</div><div class="value" id="dynoShopXp">-</div><div class="meta">Activité & économie</div></div>
      </div>
      <div class="split-2">
        <div class="card">
          <div class="section-head"><h3>Actions rapides</h3><span class="muted-sm">serveur</span></div>
          <div class="action-row">
            <button class="btn danger" onclick="panelAction('lockdown', true)">Lockdown</button>
            <button class="btn" onclick="panelAction('lockdown', false)">Unlock</button>
            <button class="btn danger" onclick="panelAction('purge_global', 20)">Purge 20</button>
            <button class="btn" onclick="loadLogsOnce()">Actualiser logs</button>
          </div>
          <div class="hint">Fonctionne seulement si le bot est connecté au serveur sélectionné et a les permissions nécessaires.</div>
        </div>
        <div class="card">
          <div class="section-head"><h3>Modules prêts</h3><span class="muted-sm">suite</span></div>
          <div class="ops-table">
            <div class="ops-row"><div><b>Automod+</b><div class="muted-sm">mentions, bad words, doublons</div></div><div>sécurité</div><div><span class="badge ok">ready</span></div></div>
            <div class="ops-row"><div><b>Tickets+</b><div class="muted-sm">panel, catégorie, transcript</div></div><div>support</div><div><span class="badge ok">ready</span></div></div>
            <div class="ops-row"><div><b>Reaction Roles</b><div class="muted-sm">liaison emoji → rôle</div></div><div>communauté</div><div><span class="badge ok">ready</span></div></div>
          </div>
        </div>
      </div>
      <div class="grid">
        <div class="card">
          <div class="section-head"><h3>Sanctions rapides</h3><span class="muted-sm">modération</span></div>
          <div class="row">
            <div style="min-width:260px"><label>Recherche membre</label><input id="member_search" placeholder="pseudo / @username / id" oninput="filterMembers()" /><div class="hint" id="members_status">Clique “Charger” pour charger la liste.</div></div>
            <div style="min-width:360px"><label>Membre</label><select id="target_select" onchange="onMemberSelect()"><option value="">— Sélectionner un membre —</option></select></div>
            <div style="min-width:260px"><label>ID (fallback)</label><input id="target" placeholder="ex: 1234567890"/></div>
            <div style="min-width:320px"><label>Raison</label><input id="reason" placeholder="optionnel"/></div>
          </div>
          <div class="row" style="margin-top:10px">
            <div style="min-width:220px"><label>Durée timeout</label><select id="timeout_preset" onchange="onTimeoutPreset()"><option value="1m">1m</option><option value="10m">10m</option><option value="30m">30m</option><option value="1h" selected>1h</option><option value="6h">6h</option><option value="12h">12h</option><option value="1d">1j</option><option value="3d">3j</option><option value="1w">1s</option><option value="2w">2s</option><option value="1mo">1mo</option><option value="3mo">3mo</option><option value="custom">Custom…</option></select></div>
            <div style="min-width:220px"><label>Custom</label><input id="timeout_custom" placeholder="ex: 1h30m" disabled /></div>
            <div style="flex:1"></div>
          </div>
        ...
        <div class="card">
          <div class="section-head"><h3>Journal rapide</h3><span class="muted-sm">live</span></div>
          <div class="console" id="dynoQuickLog">Charge le serveur pour voir un résumé des derniers logs.</div>
        </div>
      </div>
    </section>
"""
        PANEL_HTML = PANEL_HTML[:dash_start] + new_dash + PANEL_HTML[dash_end + len("</section>"):]

    PANEL_HTML = PANEL_HTML.replace(
        "</script>",
        """
async function dynoHydrateOverview(){
  try{
    const d = await api('/api/stats/overview', {k:keyVal(), g:guildVal()});
    if(d.error) return;
    const setText = (id, val) => { const el=document.getElementById(id); if(el) el.innerText = val; };
    setText('dynoMembers', d.members ?? '-');
    setText('dynoHumansBots', `${d.humans ?? '-'} / ${d.bots ?? '-'}`);
    setText('dynoRoomsRoles', `${((d.text_channels ?? 0)+(d.voice_channels ?? 0))} / ${d.roles ?? '-'}`);
    setText('dynoShopXp', `${d.shop_items ?? 0} / ${Array.isArray(d.xp_top) ? d.xp_top.length : 0}`);
  }catch(e){}
}
(function(){
  ...
  if(oldLoadAll){
    loadAll = async function(){
      await oldLoadAll();
      try{
        const botState = (document.getElementById('botState')||{}).innerText || 'unknown';
        const guilds = (document.getElementById('guildCount')||{}).innerText || '0';
        const uptime = (document.getElementById('uptime')||{}).innerText || '-';
        const connected = botState === 'connected';
        const setText = (id, val) => { const el=document.getElementById(id); if(el) el.innerText = val; };
        setText('dynoBotState', botState);
        setText('dynoGuilds', guilds);
        setText('dynoUptime', uptime);
        const badge = document.getElementById('dynoBotBadge');
        if(badge){
          badge.className = connected ? 'badge ok' : 'badge bad';
          badge.innerText = connected ? 'Connecté' : 'Offline';
        }
        await dynoHydrateOverview();
        const r = await fetch('/api/logs');
        const arr = await r.json();
        const box = document.getElementById('dynoQuickLog');
        if(box && Array.isArray(arr)){
          box.innerHTML = arr.slice().reverse().slice(0, 14).map(x => `<div>${escapeHtml(x)}</div>`).join('');
        }
      }catch(e){}
    }
  }
