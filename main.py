...
# =========================================================
# PANEL V3 UPGRADE (Dyno-inspired coherent dashboard)
# =========================================================
def upgrade_panel_v3():
    global PANEL_HTML
    if 'panel-v3-shell' in PANEL_HTML:
        return
    ...
    PANEL_HTML = PANEL_HTML.replace(
        ...
        """
    .panel-v3-shell .side{
      background:linear-gradient(180deg, rgba(10,15,26,.98), rgba(8,12,20,.96));
      width:290px;
    }
    .panel-v3-shell .logo{
      position:relative;
    ...
    .panel-v3-shell .logo:after{
      content:'';
      position:absolute; inset:0;
      background:linear-gradient(135deg, rgba(255,255,255,.15), transparent 42%);
    }
    .panel-v3-shell .nav-group{
      margin:16px 0 8px;
      color:var(--muted);
      font-size:11px;
    ...
    .panel-v3-shell .nav button{
      display:flex;
      align-items:center;
      justify-content:space-between;
      gap:12px;
    }
    .panel-v3-shell .nav button .nav-sub{
      color:var(--muted);
      font-size:11px;
    }
    .panel-v3-shell .topbar{
      gap:18px;
      align-items:stretch;
    }
    .panel-v3-shell .hero{
    ...
    .panel-v3-shell .hero-card{
      min-height:150px;
      position:relative;
      overflow:hidden;
    }
    .panel-v3-shell .hero-card:before{
      content:'';
      position:absolute; inset:-20% auto auto -10%;
      width:240px; height:240px;
      background:radial-gradient(circle, rgba(90,167,255,.18), transparent 65%);
      pointer-events:none;
    }
    .panel-v3-shell .hero-card:after{
      content:'';
      position:absolute; right:-40px; bottom:-70px;
      width:240px; height:240px;
      background:radial-gradient(circle, rgba(53,255,155,.15), transparent 64%);
      pointer-events:none;
    }
    .panel-v3-shell .eyebrow{
    ...
    .panel-v3-shell .hero h2{
      font-size:28px;
      line-height:1.05;
      margin:0 0 10px;
    }
    .panel-v3-shell .hero p{
      margin:0;
      color:#c9d7f5;
      max-width:68ch;
      line-height:1.45;
    }
    .panel-v3-shell .stats-grid{
      display:grid;
    ...
    .panel-v3-shell .stat:after{
      content:'';
      position:absolute;
      inset:auto -20px -30px auto;
      width:120px; height:120px;
      background:radial-gradient(circle, rgba(90,167,255,.12), transparent 60%);
      pointer-events:none;
    }
    .panel-v3-shell .stat .value{
      font-size:28px;
      font-weight:800;
      letter-spacing:-.03em;
      margin-top:8px;
    }
    .panel-v3-shell .stat .meta{
      margin-top:8px;
      color:var(--muted);
      font-size:12px;
    }
    .panel-v3-shell .quick-grid{
      display:grid;
      grid-template-columns:1.1fr 1fr 1fr;
      gap:14px;
      margin-top:4px;
    }
    .panel-v3-shell .section-head{
      display:flex;
      align-items:center;
      justify-content:space-between;
      gap:12px;
    ...
    .panel-v3-shell .section-head h3{
      margin:0;
      font-size:15px;
    }
    .panel-v3-shell .muted{
      color:var(--muted);
      font-size:12px;
    }
    .panel-v3-shell .mini-actions{
      display:flex;
      gap:8px;
      flex-wrap:wrap;
    }
    .panel-v3-shell .mini-actions .btn{
      min-width:unset;
      flex:none;
    }
    .panel-v3-shell .table-like{
      display:grid;
      gap:10px;
    }
    .panel-v3-shell .table-row{
      display:grid;
    ...
    .panel-v3-shell .pill-good,.panel-v3-shell .pill-warn,.panel-v3-shell .pill-bad{
      display:inline-flex; align-items:center; justify-content:center;
      padding:5px 10px; border-radius:999px; font-size:11px; font-weight:700;
      border:1px solid transparent;
    }
    .panel-v3-shell .pill-good{ background:rgba(53,255,155,.10); color:#b4ffd7; border-color:rgba(53,255,155,.18); }
    .panel-v3-shell .pill-warn{ background:rgba(255,209,102,.10); color:#ffe2a3; border-color:rgba(255,209,102,.18); }
    .panel-v3-shell .pill-bad{ background:rgba(255,59,92,.10); color:#ffcad5; border-color:rgba(255,59,92,.18); }
    .panel-v3-shell .dense input,
    .panel-v3-shell .dense select{
      padding:10px 11px;
    }
    .panel-v3-shell .empty-state{
    ...
    .panel-v3-shell .console{ min-height:240px; }
    .panel-v3-shell .side-footer{
      margin-top:18px;
      padding-top:16px;
    ...
    .panel-v3-shell .side-footer code{ color:#dce7ff; }
    @media(max-width:1180px){
      .panel-v3-shell .hero{ grid-template-columns:1fr; }
      .panel-v3-shell .stats-grid{ grid-template-columns:1fr 1fr; }
      .panel-v3-shell .quick-grid{ grid-template-columns:1fr; }
    }
    @media(max-width:760px){
      .panel-v3-shell .stats-grid{ grid-template-columns:1fr; }
      .panel-v3-shell .table-row{ grid-template-columns:1fr; }
    }
    </style>"""
    )
    PANEL_HTML = PANEL_HTML.replace('<body>', '<body class="panel-v3-shell">')
    nav_replacement = """
    <div class="nav">
      <div class="nav-group">Vue d'ensemble</div>
      <button class="active" data-tab="tab-dashboard"><span>Dashboard</span><span class="nav-sub">pilotage</span></button>
      <button data-tab="tab-analytics"><span>Analytics</span><span class="nav-sub">santé</span></button>

      <div class="nav-group">Communauté</div>
      <button data-tab="tab-config"><span>Config</span><span class="nav-sub">guild</span></button>
      <button data-tab="tab-addonsplus"><span>Automod+</span><span class="nav-sub">sécurité</span></button>
      <button data-tab="tab-moderation"><span>Modération</span><span class="nav-sub">actions</span></button>
      <button data-tab="tab-ticketsplus"><span>Tickets+</span><span class="nav-sub">support</span></button>
      <button data-tab="tab-reactionroles"><span>Reaction Roles</span><span class="nav-sub">rôles</span></button>

      <div class="nav-group">Contenu</div>
      <button data-tab="tab-embed"><span>Embeds</span><span class="nav-sub">annonces</span></button>
      <button data-tab="tab-giveaway"><span>Giveaways</span><span class="nav-sub">events</span></button>
      <button data-tab="tab-economyplus"><span>Économie+</span><span class="nav-sub">balance</span></button>

      <div class="nav-group">Outils</div>
      <button data-tab="tab-tools"><span>Outils</span><span class="nav-sub">utilitaires</span></button>
      <button data-tab="tab-logs"><span>Logs</span><span class="nav-sub">temps réel</span></button>
    </div>
    """
    PANEL_HTML = re.sub(r'<div class="nav">.*?</div>\s*\n\n    <div style="margin-top:14px" class="hint">', nav_replacement + '\n\n    <div class="side-footer">', PANEL_HTML, count=1, flags=re.S)
    PANEL_HTML = PANEL_HTML.replace('<b>Connexion</b><br/>', '<b>Accès admin</b><br/>')
    PANEL_HTML = PANEL_HTML.replace('</aside>', '</div></aside>', 1)
    PANEL_HTML = PANEL_HTML.replace(
        '<div class="card" style="min-width:260px">',
        '<div class="card" style="min-width:300px"><div class="section-head"><h3>État du bot</h3><span class="pill-warn" id="healthBadge">En attente</span></div>',
        1
    )
    PANEL_HTML = PANEL_HTML.replace('<div class="title">Infos</div>\n        <div class="hint">', '<div class="hint">', 1)
    ...
    <section id="tab-dashboard" class="tab active">
      <div class="hero">
        <div class="card hero-card">
          <div class="eyebrow"><span class="dot"></span><span>Leviathan Control Center</span></div>
          <h2>Un panel plus propre, plus lisible et pensé pour piloter ton serveur.</h2>
          <p>Vue globale, modération rapide, santé du bot, sécurité, tickets, économie, rôles et analytics. Charge ton serveur et tout le dashboard se met à jour.</p>
          <div class="mini-actions" style="margin-top:18px">
            <button class="btn primary" onclick="loadAll()">Actualiser tout</button>
            <button class="btn" onclick="fetchInfo()">Rafraîchir état</button>
            <button class="btn" onclick="loadOverview()">Voir overview</button>
          </div>
        </div>
        <div class="card hero-card">
          <div class="section-head"><h3>Résumé opérationnel</h3><span class="muted">live</span></div>
          <div class="table-like">
            <div class="table-row">
              <div><b>Connexion bot</b><div class="muted">Présence Discord et accès serveur</div></div>
              <div id="dashBotState">unknown</div>
              <div><span class="pill-warn" id="dashBotBadge">En attente</span></div>
            </div>
            <div class="table-row">
              <div><b>Guilds visibles</b><div class="muted">Serveurs disponibles dans le panel</div></div>
              <div id="dashGuilds">0</div>
              <div><span class="pill-good">Panel actif</span></div>
            </div>
            <div class="table-row">
              <div><b>Uptime</b><div class="muted">Temps de fonctionnement</div></div>
              <div id="dashUptime">-</div>
              <div><span class="pill-good">OK</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="stats-grid">
        <div class="card stat">
          ...
          <div class="meta">Population totale du serveur</div>
        </div>
        <div class="card stat">
          <div class="title">Humains / Bots</div>
          <div class="value" id="statHumansBots">-</div>
          <div class="meta">Répartition de la communauté</div>
        </div>
        <div class="card stat">
          <div class="title">Salons / Rôles</div>
          <div class="value" id="statRoomsRoles">-</div>
          <div class="meta">Structure du serveur</div>
        </div>
        <div class="card stat">
          <div class="title">Shop / XP top</div>
          <div class="value" id="statShopXp">-</div>
          <div class="meta">Économie et activité</div>
        </div>
      </div>

      <div class="quick-grid">
        <div class="card">
          <div class="section-head"><h3>Actions rapides</h3><span class="muted">serveur</span></div>
          ...
          <div class="hint">Utilise ces actions seulement quand le bot est connecté au serveur sélectionné.</div>
        </div>

        <div class="card dense">
          <div class="section-head"><h3>Santé & API</h3><span class="muted">technique</span></div>
          <div class="console" id="healthBox">Clique “Charger” pour afficher l’état.</div>
        </div>

        <div class="card">
          <div class="section-head"><h3>Raccourcis utiles</h3><span class="muted">workflow</span></div>
          <div class="table-like">
            <div class="table-row">
              <div><b>Automod+</b><div class="muted">mentions, bad words, ghost ping</div></div>
              <div>Onglet</div>
              <div><span class="pill-good">Ready</span></div>
            </div>
            <div class="table-row">
              <div><b>Tickets+</b><div class="muted">catégorie, transcript, panel</div></div>
              <div>Onglet</div>
              <div><span class="pill-good">Ready</span></div>
            </div>
            <div class="table-row">
              <div><b>Reaction Roles</b><div class="muted">liaison emoji → rôle</div></div>
              <div>Onglet</div>
              <div><span class="pill-good">Ready</span></div>
            </div>
          </div>
      ...
      <div class="grid" style="margin-top:14px">
        <div class="card">
          <div class="section-head"><h3>Sanctions rapides</h3><span class="muted">modération</span></div>
          <div class="row">
            <div style="min-width:260px">
              <label>Recherche membre</label>
              <input id="member_search" placeholder="pseudo / @username / id" oninput="filterMembers()" />
        ...
        <div class="card">
          <div class="section-head"><h3>Journal rapide</h3><span class="muted">live</span></div>
          <div class="console" id="dashboardLogBox">Charge le serveur pour voir un résumé des derniers logs.</div>
        </div>
      </div>
    </section>
"""
    ...
    PANEL_HTML = PANEL_HTML.replace(
        "</script>",
        """
async function hydrateDashboardV3FromOverview(){
  try{
    const d = await api('/api/stats/overview',{k:keyVal(),g:guildVal()});
    if(d.error){ return; }
    const set = (id,val)=>{ const el=document.getElementById(id); if(el) el.innerText=val; };
    set('statMembers', d.members ?? '-');
    set('statHumansBots', `${d.humans ?? '-'} / ${d.bots ?? '-'}`);
    set('statRoomsRoles', `${((d.text_channels ?? 0)+(d.voice_channels ?? 0))} / ${d.roles ?? '-'}`);
    set('statShopXp', `${d.shop_items ?? 0} / ${(Array.isArray(d.xp_top)?d.xp_top.length:0)}`);
  }catch(e){}
}

(function(){
  const _oldFetchInfo = typeof fetchInfo === 'function' ? fetchInfo : null;
  if(_oldFetchInfo){
    fetchInfo = async function(){
      ...
      const dashBadge = document.getElementById('dashBotBadge');
      setText('dashBotState', botState);
      setText('dashGuilds', guilds);
      setText('dashUptime', uptime);
      if(badge){
        badge.className = connected ? 'pill-good' : 'pill-bad';
        badge.innerText = connected ? 'Connecté' : 'Offline';
      }
      if(dashBadge){
        dashBadge.className = connected ? 'pill-good' : 'pill-bad';
        dashBadge.innerText = connected ? 'Connecté' : 'Offline';
      }
      await hydrateDashboardV3FromOverview();
      try{
        const r = await fetch('/api/logs');
        const arr = await r.json();
        const box = document.getElementById('dashboardLogBox');
        if(box && Array.isArray(arr) && arr.length){
          box.innerHTML = arr.slice().reverse().slice(0,12).map(x=>`<div>${escapeHtml(x)}</div>`).join('');
        }
      }catch(e){}
    }
  }
